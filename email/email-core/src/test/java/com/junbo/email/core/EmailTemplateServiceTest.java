...
public class EmailTemplateServiceTest extends BaseTest {
    ...
    public void testCreate() throws Exception {
        EmailTemplate emailTemplate = templateService.postEmailTemplate(template).wrapped().get();
        Assert.assertNotNull(emailTemplate,"Email template should not be null");
    }

    @Test
    public void testGet() throws Exception {
        EmailTemplate saveTemplate = templateService.postEmailTemplate(template).wrapped().get();
        EmailTemplate emailTemplate = templateService.getEmailTemplate(saveTemplate.getId().getValue()).wrapped().get();
        ...
        EmailTemplate saveTemplate2 = templateService.postEmailTemplate(template2).wrapped().get();
        Results<EmailTemplate> list = templateService.getEmailTemplates(null).wrapped().get();
