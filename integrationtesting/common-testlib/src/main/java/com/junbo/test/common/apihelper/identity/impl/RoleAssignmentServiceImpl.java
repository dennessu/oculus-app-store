...
public class RoleAssignmentServiceImpl extends HttpClientBase implements RoleAssignmentService {
    ...
    public RoleAssignment postRoleAssignment(RoleAssignment roleAssignment, int expectedResponseCode) throws Exception {
        String responseBody = restApiCall(HTTPMethod.POST, roleAssignmentURL, roleAssignment, expectedResponseCode);
        return new JsonMessageTranscoder().decode(new TypeReference<RoleAssignment>() {}, responseBody);
    }
    ...
    public Results<RoleAssignment> getRoleAssignments(int expectedResponseCode) throws Exception {
        String responseBody = restApiCall(HTTPMethod.GET, roleAssignmentURL, expectedResponseCode);
        return new JsonMessageTranscoder().decode(new TypeReference<Results<RoleAssignment>>() {}, responseBody);
    }
    ...
    public RoleAssignment updateRoleAssignment(RoleAssignment roleAssignment, int expectedResponseCode) throws Exception {
        String putUrl = roleAssignmentURL + "/" + roleAssignment.getId().toString();
        String responseBody = restApiCall(HTTPMethod.PUT, putUrl, roleAssignment, expectedResponseCode);
        return new JsonMessageTranscoder().decode(new TypeReference<RoleAssignment>() {}, responseBody);
    }
