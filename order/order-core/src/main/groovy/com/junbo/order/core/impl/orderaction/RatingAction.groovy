...
class RatingAction implements Action {
    ...
    Promise<ActionResult> execute(ActionContext actionContext) {
        ...
        return serviceContext.shippingAddress.syncThen { ShippingAddress sa ->
            serviceContext.ratingFacade.rateOrder(order, sa?.country).syncRecover {
                Throwable throwable ->
                ...
                if (ratingResult == null) {
                    serviceContext.orderRepository.createOrderEvent(
                            CoreBuilder.buildOrderEvent(order.id,
                                    com.junbo.order.spec.model.OrderActionType.RATE, EventStatus.ERROR))
                } else {
                    ...
                    serviceContext.orderRepository.createOrderEvent(
                            CoreBuilder.buildOrderEvent(order.id,
                                    com.junbo.order.spec.model.OrderActionType.RATE, ratingStatus))
                    return context
                }
            }
