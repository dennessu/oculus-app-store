...
public class SubledgerItemResourceClientProxy implements com.junbo.order.spec.resource.SubledgerItemResource {
    ...
    public SubledgerItemResourceClientProxy(AsyncHttpClient client, MessageTranscoder transcoder, PathParamTranscoder pathParamTranscoder,
    ...
    public Promise<com.junbo.order.spec.model.SubledgerItem> createSubledgerItem(com.junbo.order.spec.model.SubledgerItem subledgerItem, javax.ws.rs.core.HttpHeaders headers) {
    
        javax.ws.rs.core.UriBuilder __uriBuilder = UriBuilder.fromUri(__target);
        __uriBuilder.path("subledger-items");
        ...
        __requestBuilder.setBody(__transcoder.encode(subledgerItem));
        ...
        return __future.then(new Promise.Func<Response, Promise<com.junbo.order.spec.model.SubledgerItem>>() {
            @Override
            public Promise<com.junbo.order.spec.model.SubledgerItem> apply(Response response) {
                if (response.getStatusCode() / 100 == 2) {
                    try {
                        return Promise.pure(__transcoder.<com.junbo.order.spec.model.SubledgerItem>decode(new TypeReference<com.junbo.order.spec.model.SubledgerItem>() {}, response.getResponseBody()));
                    } catch (java.io.IOException ex) {
                         throw new RuntimeException(ex);
                    }
                } else {
