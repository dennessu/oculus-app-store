...
import com.junbo.idea.codenarc.model.ConfigurationType;
...
/**
 * A manager for CodeNarc plug-in configuration.
 */
@State(
        name = CodeNarcConstants.ID_PLUGIN,
        storages = {
                @Storage(id = "default", file = StoragePathMacros.PROJECT_FILE),
                @Storage(id = "dir", file = StoragePathMacros.PROJECT_CONFIG_DIR + "/codenarc-idea.xml", scheme = StorageScheme.DIRECTORY_BASED)
        }
)
public final class CodeNarcConfiguration implements ExportableComponent,
        PersistentStateComponent<CodeNarcConfiguration.ProjectSettings> {

    private static final Log LOG = LogFactory.getLog(CodeNarcConfiguration.class);
    ...
    private static final String CHECK_NONGROOVY_FILES = "check-nongroovy-files";
    ...
    private static final String ALL_CHECKS_CONFIG = "/all_rules.groovy";
     ...
     */
    public CodeNarcConfiguration(final Project project) {
        if (project == null) {
        ...
        final ConfigurationLocation allChecks = ConfigurationLocationFactory.create(project, ConfigurationType.CLASSPATH,
                ALL_CHECKS_CONFIG, resources.getString("file.default.description"));
        presetLocations.add(allChecks);
    }
    ...
    public File[] getExportFiles() {
        return new File[]{PathManager.getOptionsFile("codenarc-idea_project_settings")};
    }

    @NotNull
    public String getPresentableName() {
        return "CodeNarc-IDEA Project Settings";
    }
            ...
            for (ConfigurationLocation configurationLocation : configurationLocations) {
                ...
                try {
                    ...
                    final MessageFormat message = new MessageFormat(
                            IDEAUtilities.getResource("codenarc.could-not-read-properties",
                                    "Properties could not be read from the CodeNarc configuration file from {0}."));
    ...
    public boolean isScanningNonGroovyFiles() {
        final String propertyValue = storage.get(CHECK_NONGROOVY_FILES);
        return propertyValue != null && Boolean.valueOf(propertyValue);
    }

    public void setScanningNonGroovyFiles(final boolean scanNonGroovyFiles) {
        storage.put(CHECK_NONGROOVY_FILES, Boolean.toString(scanNonGroovyFiles));
    }
    ...
    private String untokenisePath(final String path) {
        ...
        for (String prefix : new String[]{CodeNarcConstants.PROJECT_DIR, CodeNarcConstants.LEGACY_PROJECT_DIR}) {
    ...
    private String tokenisePath(final String path) {
        ...
        if (projectPath != null) {
            ...
            if (path.startsWith(projectPathAbs)) {
                return CodeNarcConstants.PROJECT_DIR + path.substring(
