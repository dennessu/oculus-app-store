...
import com.junbo.idea.codenarc.ConfigurationListener;
...
/**
 * The tool window for CodeNarc scans.
 */
public class CodeNarcToolWindowPanel extends JPanel implements ConfigurationListener {

    /**
     * Logger for this class.
     */
    private static final Log LOG = LogFactory.getLog(CodeNarcToolWindowPanel.class);

    private static final String MAIN_ACTION_GROUP = "CodeNarcPluginActions";
    private static final String TREE_ACTION_GROUP = "CodeNarcPluginTreeActions";
    private static final String DEFAULT_OVERRIDE = IDEAUtilities.getResource("plugin.toolwindow.default-file", "<As configured>");

    private final CodeNarcPlugin codeNarcPlugin;
     ...
     */
    public CodeNarcToolWindowPanel(final ToolWindow toolWindow, final Project project) {
        super(new BorderLayout());

        this.toolWindow = toolWindow;
        this.project = project;

        codeNarcPlugin = project.getComponent(CodeNarcPlugin.class);
        if (codeNarcPlugin == null) {
            throw new IllegalStateException("Couldn't get CodeNarc plugin");
        }

        configurationChanged();
        codeNarcPlugin.getConfiguration().addConfigurationListener(this);
        ...
        final ActionToolbar mainToolbar = ActionManager.getInstance().createActionToolbar(
                CodeNarcConstants.ID_TOOLWINDOW, mainActionGroup, false);
        ...
        final ActionToolbar treeToolbar = ActionManager.getInstance().createActionToolbar(
                CodeNarcConstants.ID_TOOLWINDOW, treeActionGroup, false);
    ...
    @Nullable
    public static CodeNarcToolWindowPanel panelFor(final Project project) {
        ...
        final ToolWindow toolWindow = toolWindowManager.getToolWindow(CodeNarcConstants.ID_TOOLWINDOW);
        if (toolWindow == null) {
            LOG.debug("Couldn't get tool window for ID " + CodeNarcConstants.ID_TOOLWINDOW);
            return null;
        }

        for (Content currentContent : toolWindow.getContentManager().getContents()) {
            if (currentContent.getComponent() instanceof CodeNarcToolWindowPanel) {
                return ((CodeNarcToolWindowPanel) currentContent.getComponent());
            }
        }

        LOG.debug("Could not find tool window panel on tool window with ID " + CodeNarcConstants.ID_TOOLWINDOW);
        return null;
    }
    ...
    public void configurationChanged() {
        ...
        for (ConfigurationLocation configurationLocation : codeNarcPlugin.getConfiguration().getConfigurationLocations()) {
     ...
     */
    public CodeNarcToolWindowPanel expandTree() {
        expandTree(3);
     ...
     */
    public CodeNarcToolWindowPanel displayResults(final Map<PsiFile, List<ProblemDescriptor>> results) {
        treeModel.setModel(results);
