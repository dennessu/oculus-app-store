...
var Provider = require('../lib/api_provider').Catalog;
var Cache = require('../lib/cache').Cache;
var CacheTypes = require('../lib/cache').CacheTypes;
...
/*
    result: offer array
*/
Catalog.prototype._GetOffers = function(offerId, callback){

    var resultList = new Array();
    var apiProvider = new Provider(this.Host, this.Port);
    ...
    Async.waterfall([
            ...
            function(cb){
                if(offerId != null){
                    apiProvider.GetOfferById(offerId, function (result) {
                    ...
                    apiProvider.GetOffers(function (result) {
                        if (result.StatusCode.toString()[0] == 2) {
            ...
            function(offers, cb){
                ...
                for(var i = 0; i < offers.length; ++i){
                    ...
                    (function(offerId, revisionId){
                        revisionFunList.push(function(cb){
                            apiProvider.GetOfferRevisionById(revisionId, function(result){
                                if(result.StatusCode.toString()[0] == 2){
                                    cb(null, result);
                                }else{
                ...
                Async.parallel(revisionFunList, function(err, resultList){
                    if(err){
                        cb("Get offer revision failed!", resultList);
                    }
                            ...
                            if (revisionObj.price.priceType == "CUSTOM") {
                                ...
                                (function (tierOfferId, priceTierId) {
                                    priceTiersFunList.push(function (cb) {
                                        apiProvider.GetPriceTierById(priceTierId, function (tierResult) {
        ...
        function(err, result){
            ...
            (new Cache()).AddArray(CacheTypes.OFFERS, "id", resultList);
            callback(null, resultList);
        });
};

Catalog.prototype.GetOffers = function(callback){
    var resultList = this.Cache.GetArray(CacheTypes.OFFERS);
    if(resultList != null) {
        callback(null, resultList);
        return;
    }

    this._GetOffers(null, callback);
};

Catalog.prototype.GetOfferById = function(offerId, callback){
    var resultList = this.Cache.Get(CacheTypes.OFFERS, offerId);
    if(resultList != null) {
        callback(null, resultList);
        return;
    }

    this._GetOffers(offerId, function(err, result){
        if(err){
            callback(err, result);
        }
