...
def readParams():
    # Validate command
    def printUsage():
        error("Usage: python ./copydata.py {from} {to}\n")
    ...
    print "copying data from %s to %s" % (source, target)

    return (source, target)


def copydata(source, target):
    startKey = ""
    batchSize = 1000
    while True:
        allDocs = curlJson(source + '/_all_docs?limit=%d&startkey="%s"&include_docs=true' % (batchSize, startKey), "GET")
        docs = []
