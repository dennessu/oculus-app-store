            ...
            public Promise<${returnType}> apply() {
                return __service.${methodName}([#list parameters as parameter]${parameter.paramName}[#if parameter_has_next], [/#if][/#list]).then(new Promise.Func<${returnType}, Promise<${returnType}>>() {
                    ...
                    public Promise<${returnType}> apply(${returnType} __result) {
                        LOGGER.info("(InProc) Method: ${methodName} base: " + __target + " uri:" + __uriBuilder.toTemplate().replace(__target, "") + " duration: " + (System.currentTimeMillis() - __startTime) + "ms " + __startDate.toString() + " machineName: " + __machineName);
...
[/#if]
    ...
    try {
        ...
        LOGGER.error("(Remote) Method: ${methodName} base: " + __target + " uri:" + __uriBuilder.toTemplate().replace(__target, "") + " duration: " + (System.currentTimeMillis() - __startTime) + "ms " + __startDate.toString() + " machineName: " + __machineName);
    ...
    return __future.then(new Promise.Func<Response, Promise<${returnType}>>() {
        ...
        LOGGER.info("(Remote) Method: ${methodName} base: " + __target + " uri:" + __uriBuilder.toTemplate().replace(__target, "") + " duration: " + (System.currentTimeMillis() - __startTime) + "ms " + __startDate.toString() + " machineName: " + __machineName);
