...
for var in $ENV/apps-2.txt $ENV/crypto-apps-2.txt; do
    for p in `cat $var`; do
        if [[ ! -z "$p" && ! $p == \#* ]]; then
...
cp $SOURCETREE_HOME/apphost/apphost-cli/build/distributions/$APP_NAME.zip /home/silkcloud
for var in $ENV/crypto-apps-2.txt $ENV/apps-2.txt; do
    for p in `cat $var`; do
        if [[ ! -z "$p" && ! $p == \#* ]]; then
...
./pforeach-here.sh $ENV/crypto-apps-2.txt << EOF
...
./pforeach-here.sh $ENV/apps-2.txt << EOF
