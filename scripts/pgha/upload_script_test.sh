...
scp -o "StrictHostKeyChecking no" -r ./ $DEVOPS_ACCOUNT@$MASTER_HOST:$DEPLOYMENT_PATH
...
scp -o "StrictHostKeyChecking no" -r ./ $DEVOPS_ACCOUNT@$SLAVE_HOST:$DEPLOYMENT_PATH
...
function uploadBCP {
ssh -o "StrictHostKeyChecking no" $DEVOPS_ACCOUNT@$BCP_HOST << ENDSSH
sudo mkdir -p /etc/silkcloud
sudo touch /etc/silkcloud/configuration.properties
sudo chmod 666 /etc/silkcloud/configuration.properties
sudo echo "environment=$ENVIRONMENT" > /etc/silkcloud/configuration.properties
...
scp -o "StrictHostKeyChecking no" -r ./ $DEVOPS_ACCOUNT@$BCP_HOST:$DEPLOYMENT_PATH
...
scp -o "StrictHostKeyChecking no" -r ./ $DEVOPS_ACCOUNT@$REPLICA_HOST:$DEPLOYMENT_PATH
