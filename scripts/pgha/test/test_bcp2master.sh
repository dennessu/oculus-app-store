...
$PGBIN_PATH/psql postgres -h $BCP_HOST -p $SLAVE_DB_PORT -c "DELETE FROM msx;"

# insert against BCP server
echo 'insert 9 records to postgres.msx table on slave'
for k in $(seq 1 9)
do
    $PGBIN_PATH/psql postgres -h $BCP_HOST -p $SLAVE_DB_PORT -c "INSERT INTO msx (id) VALUES ($k);"
done
...
$PGBIN_PATH/psql postgres -h $SLAVE_HOST -p $SLAVE_DB_PORT -c "SELECT max(id) FROM msx"
$PGBIN_PATH/psql postgres -h $SLAVE_HOST -p $SLAVE_DB_PORT -c "SELECT count('x') FROM msx"
