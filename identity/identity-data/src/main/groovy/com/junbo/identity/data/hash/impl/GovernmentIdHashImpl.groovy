 ...
 */
@CompileStatic
class GovernmentIdHashImpl implements PiiHash {

    private String salt

    @Override
    boolean handles(String piiType) {
        if (piiType == UserPersonalInfoType.GOVERNMENT_ID.toString()) {
            return true
        }
        return false
    }
    ...
    String generateHash(JsonNode jsonNode) {
        UserGovernmentID userGovernmentID = (UserGovernmentID)JsonHelper.jsonNodeToObj(jsonNode, UserGovernmentID)
        assert userGovernmentID != null
