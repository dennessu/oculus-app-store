 ...
 */
@CompileStatic
class PhoneAdvanceFilter extends HBIPIIAdvanceFilter {
    @Override
    boolean handles(UserPersonalInfo userPersonalInfo) {
        return userPersonalInfo.type == UserPersonalInfoType.PHONE.toString()
    }
    ...
    UserPersonalInfo getFilter(UserPersonalInfo userPersonalInfo) {
        ...
        phoneNumber.info = mask(phoneNumber.info)
        userPersonalInfo.value = ObjectMapperProvider.instance().valueToTree(phoneNumber)
